...
export default function FeatureGrid() {
  return (
    <section className="py-20 text-white relative" aria-labelledby="why-heading">
            ...
            return (
              <Reveal key={f.id} delay={i * 0.06}>
                <div className="p-6 transition-all hover:-translate-y-1">
                  <span className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-teal-500/20 text-teal-300 mb-4 border border-teal-500/30">
                    <Icon className="w-5 h-5" aria-hidden="true" />
                  </span>
                  <h3 className="font-heading text-lg font-extrabold text-white mb-2">{f.title}</h3>
                  <p className="text-sm leading-relaxed text-slate-300 font-medium">{f.text}</p>
                </div>
              </Reveal>
            );
          })}
